...
\begin{frame}[fragile]{Lorentz vector functions apply to arrays and jagged arrays}
\vspace{0.5 cm}
...
\item {\tt \small Table} to hold contiguous or non-contiguous arrays of each component
...
\item<3-> A lot of time spent in validity checks (e.g.\ ensure that {\tt \small starts} are all within {\tt \small content}). Need to avoid redundant checks without removing safety.
